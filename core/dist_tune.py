...
def dist_tune():
    ...
    for epoch in range(param.finetune_num_epochs):
        ...
        if epoch_classify_loss + epoch_dist_loss < min_loss:
            ...
            remove_glob(f"{param.dist_tune_encoder_weight}_*")
            remove_glob(f"{param.dist_tune_classifier_weight}_*")
            torch.save(encoder.module.state_dict(), f"{param.dist_tune_encoder_weight}_{epoch}")
            torch.save(classifier.module.state_dict(), f"{param.dist_tune_classifier_weight}_{epoch}")
