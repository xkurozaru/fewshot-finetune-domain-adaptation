...
def main():
    ...
    for epoch in param.test_epochs:
        ...
        encoder.load_state_dict(torch.load(f"{param.triplet_tune_encoder_weight}_epoch_{epoch}"))
        classifier.load_state_dict(torch.load(f"{param.triplet_tune_classifier_weight}_epoch_{epoch}"))
