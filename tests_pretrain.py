...
def main():
    ...
    for epoch in range(10, 101, 10):
        ...
        encoder.load_state_dict(torch.load(f"{param.pretrain_encoder_weight}_epoch_{epoch}"))
        classifier.load_state_dict(torch.load(f"{param.pretrain_classifier_weight}_epoch_{epoch}"))
        ...
        plt.savefig(f"{save_dir}/pretrain_report_{epoch}.png")
        ...
        plt.savefig(f"{save_dir}/pretrain_confusion_matrix_{epoch}.png")

    with open("result/pretrain_scores.csv", "a") as f:
