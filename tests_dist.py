...
def main():
    ...
    for epoch in param.test_epochs:
        ...
        encoder.load_state_dict(torch.load(f"{param.dist_tune_encoder_weight}_epoch_{epoch}"))
        classifier.load_state_dict(torch.load(f"{param.dist_tune_classifier_weight}_epoch_{epoch}"))
        ...
        plt.savefig(f"{save_dir}/dist_tune_report_{epoch}.png")
        ...
        plt.savefig(f"{save_dir}/dist_tune_confusion_matrix_{epoch}.png")

    with open("result/dist_tune_scores.csv", "a") as f:
